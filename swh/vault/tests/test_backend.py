...
def fake_cook(backend, bundle_type, swhid, result_content, sticky=False):
    backend.create_task(bundle_type, swhid, sticky)
    backend.cache.add(bundle_type, swhid, result_content)
...
def test_create_fail_nonexisting_object(swh_vault, mocker):
    cooker = mocker.MagicMock()
    cooker.check_exists.side_effect = ValueError("Nothing here.")
    cooker_class = mocker.MagicMock()
    cooker_class.return_value = cooker
    mocker.patch("swh.vault.backend.get_cooker_cls").return_value = cooker_class
...
def test_create_scheduler_rpc_failure(swh_vault, mocker):
    send_task = mocker.patch.object(swh_vault, "_send_task")
    send_task.side_effect = APIError("Could not connect to scheduler RPC service")
    # exception should bubble up when requesting to cook a bundle
    with pytest.raises(APIError):
        cooking_info = swh_vault.cook(TEST_TYPE, TEST_SWHID)

    mocker.stop(send_task)
...
def test_cook_email_pending_done(swh_vault, mocker):
    madd = mocker.patch.object(swh_vault, "add_notif_email")
    msend = mocker.patch.object(swh_vault, "send_notification")
...
def test_send_all_emails(swh_vault, mocker):
    ...
    m = mocker.patch.object(swh_vault, "_smtp_send")
...
def test_send_all_emails_custom_notif(swh_vault_custom_notif, mocker):
...
def test_send_email_error_send_failed(swh_vault, mocker):
    reports = []
    init_sentry(SENTRY_DSN, extra_kwargs={"transport": reports.append})
...
def test_fetch(swh_vault):
    ...
    fake_cook(swh_vault, TEST_TYPE, TEST_SWHID, b"content")

    info = swh_vault.progress(TEST_TYPE, TEST_SWHID)
    access_ts_before = info["ts_last_access"]

    assert swh_vault.fetch(TEST_TYPE, TEST_SWHID) == b"content"
...
def test_cache_expire_oldest(swh_vault, sample_data):
    r = range(1, 10)
    inserted = {}
    for i in r:
        sticky = i == 5
        content = b"content%s" % str(i).encode()
        rev_swhid = CoreSWHID.from_string(str(TEST_SWHID)[:-1] + str(i))
        revision = attr.evolve(sample_data.revision, id=rev_swhid.object_id)
        swh_vault.storage.revision_add([revision])
        fake_cook(
            swh_vault,
            TEST_TYPE,
        ...
        inserted[i] = rev_swhid

    swh_vault.update_access_ts(TEST_TYPE, inserted[2])
    swh_vault.update_access_ts(TEST_TYPE, inserted[3])
    swh_vault.cache_expire_oldest(n=4)

    should_be_still_here = {2, 3, 5, 8, 9}
    for i in r:
        assert swh_vault.is_available(TEST_TYPE, inserted[i]) == (
            i in should_be_still_here
        )


def test_cache_expire_until(swh_vault, sample_data):
    r = range(1, 10)
    inserted = {}
    for i in r:
        sticky = i == 5
        ...
        revision = attr.evolve(sample_data.revision, id=rev_swhid.object_id)
        swh_vault.storage.revision_add([revision])
        fake_cook(swh_vault, TEST_TYPE, rev_swhid, content, sticky)
        inserted[i] = rev_swhid
...
def test_send_failure_email(swh_vault, mocker):
...
def test_download_url_cache_pathslicing_backend(swh_vault):
    ...
    assert swh_vault.download_url(TEST_TYPE, TEST_SWHID) is None
